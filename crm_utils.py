...
# ============================================================
#  PROYECTOS — CRUD BÁSICO (con cache en lecturas)
# ============================================================

@st.cache_data(ttl=120, show_spinner=False)
def get_clientes():
    """
    Devuelve los clientes desde Firestore en un DataFrame.
    Se cachea 120s para reducir lecturas.
    """
    db = _get_db()
...
def get_proyectos():
    """
    Devuelve los proyectos desde Firestore en un DataFrame.
    Se cachea 120s para reducir lecturas.
    """
    db = _get_db()
...
def _clear_proyectos_cache():
    """Invalidar cache de proyectos tras operaciones de escritura."""
    try:
        get_proyectos.clear()
    except Exception:
        pass
...
def _clear_clientes_cache():
    """Invalidar cache de clientes tras operaciones de escritura."""
    try:
        get_clientes.clear()
    except Exception:
        pass
...
def delete_all_proyectos() -> int:
    """
    Borra todos los documentos de la colección 'proyectos'.
    Usado por el botón 'Borrar todos los proyectos' en app.py.
    """
    db = _get_db()
    docs = db.collection("proyectos").stream()
