...
def render_proyectos():
    ...
    with st.spinner("Cargando proyectos…"):
        try:
            proyectos = get_proyectos()  # cacheado en crm_utils
        except Exception as e:
            st.error("❌ Error cargando proyectos (posible cuota agotada).")
            st.code(str(e))
            return
...
def _vista_tabla(df_filtrado):
    ...
    # Definir tabla UI
    df_ui = df_raw.drop(columns=["id"]).copy()
    ...
    st.markdown(
        ...
        "Selecciona una o varias obras y usa los iconos superiores."
        "</p>",
    ...
    with actions_placeholder.container():
        ...
        with col_delete:
            if st.button("🗑️", help="Borrar proyectos seleccionados"):
                ...
                if not marcados:
                    st.warning("No hay proyectos marcados.")
                else:
                    eliminados = 0
                    for i in marcados:
                        try:
...
def _open_nuevo_dialog():
    ...
    with st.form("nuevo_proyecto_form"):
        ...
        cliente = st.text_input("Cliente / Promotora")
        estado = st.selectbox(
            "Estado",
            [
...
def _open_edit_dialog(row_data, proyecto_id):
    ...
    with st.form(f"editar_{proyecto_id}"):
        ...
        cliente = st.text_input("Cliente / Promotora", row_data.get("cliente"))
        estado = st.selectbox(
            "Estado",
            [
                "Detectado", "Seguimiento", "En Prescripción",
